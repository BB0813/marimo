...
class TestExportManifestCoversReads:
    """A warm session reads from the cache instead of writing it; the
    export manifest must cover those reads or `--execute` bundles an
    incomplete sidecar on re-export."""

    def test_get_and_hit_mark_keys(self, tmp_path: Path) -> None:
        from marimo._save.loaders.lazy import LazyStore
        from marimo._save.stores.file import FileStore

        store = LazyStore(FileStore(save_path=str(tmp_path)))
        assert store.put("lazy/a.bin", b"a")
        assert store.export_manifest() == ["lazy/a.bin"]

        # Fresh store over the same directory: nothing written yet.
        warm = LazyStore(FileStore(save_path=str(tmp_path)))
        assert warm.export_manifest() == []
        assert warm.get("lazy/a.bin") == b"a"
        assert warm.export_manifest() == ["lazy/a.bin"]

        another = LazyStore(FileStore(save_path=str(tmp_path)))
        assert another.hit("lazy/a.bin")
        assert another.export_manifest() == ["lazy/a.bin"]

    def test_clear_unmarks(self, tmp_path: Path) -> None:
        from marimo._save.loaders.lazy import LazyStore
        from marimo._save.stores.file import FileStore

        store = LazyStore(FileStore(save_path=str(tmp_path)))
        store.put("lazy/a.bin", b"a")
        store.clear("lazy/a.bin")
